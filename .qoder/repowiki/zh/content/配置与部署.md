...
## 更新摘要
**所做更改**
- 新增完整的iOS平台支持章节，包括wry补丁、iOS生成系统和移动部署能力
- 更新Tauri v2配置说明，涵盖跨平台构建选项
- 增强移动端部署流程，包含真机调试和发布指南
- 添加iOS特定的环境变量和安全配置说明
- 更新构建脚本和依赖管理，支持多平台编译
...
**更新** 新增了对iOS平台的完整支持，包括wry WebView补丁、iOS项目生成系统和移动设备部署能力。
...
## 架构总览
下图展示了从前端到 Rust 后端的调用链路与数据流，以及iOS平台的集成方式：
...
- 打包选项
  ...
  - **新增** iOS 特定配置：developmentTeam 和 minimumSystemVersion 设置。
- 构建前后命令
  - beforeDevCommand 与 beforeBuildCommand 分别用于启动前端开发与构建流程，devUrl 指向本地开发端口。

**更新** 新增了iOS平台的专门配置，包括开发团队ID和最低系统版本要求。
...
**新增** 完整的iOS平台支持，解决了多个技术挑战并提供完整的开发部署流程。
...
**更新** 增强了iOS平台的开发环境配置，支持真机调试和生产部署。
...
## 结论
hq-kanban-app 基于 Tauri v2 + Vue 3 + Vite + TypeScript 的现代桌面与移动应用架构，具备清晰的配置结构与强大的跨平台扩展性。通过合理的窗口与安全策略、严格的 TS 编译与 Vite 构建、以及 Rust 侧的高效网络处理，可实现桌面端和移动端的稳定运行。新增的iOS平台支持包括wry WebView补丁、xcodegen项目生成系统和完整的部署流程，使应用能够同时在桌面设备和移动设备上运行。建议在生产环境中引入环境变量管理、自动更新与统一的版本管理，以提升安全性与维护性。

**更新** 强调了新增的iOS平台支持能力和跨平台部署优势。
...
- 平台特定设置
  ...
  - **新增** iOS：需要 Xcode、Apple 开发者账号、iOS SDK；配置 developmentTeam 和 Bundle ID。
- 发布
  - 将生成的安装包上传至分发服务器；如需自动更新，配置 Tauri Updater 与签名。
  - **新增** iOS：支持通过 App Store Connect 发布到 Apple App Store 或使用 TestFlight 进行内测。
