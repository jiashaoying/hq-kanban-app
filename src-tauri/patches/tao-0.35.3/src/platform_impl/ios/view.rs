...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // 修复：原实现用 `Retained::as_ptr(&config)` 返回借用指针，函数返回后 Retained
      // 被 drop 并 release，UISceneConfiguration 随即销毁，UIKit 拿到悬垂指针。
      // 后续 `- [UIApplication _connectUISceneFromFBSScene:]` 取 delegateClass 时
      // objc_retain 到已释放对象，表现为启动 SIGSEGV（EXC_BAD_ACCESS / signal 11）。
      // delegate 方法返回值的所有权是 +0，应交给 autorelease pool。
