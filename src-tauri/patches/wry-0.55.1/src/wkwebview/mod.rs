...
use objc2_foundation::{
  ...
  NSObjectNSKeyValueCoding, NSObjectProtocol, NSProcessInfo, NSString, NSUTF8StringEncoding, NSURL, NSUUID,
};
#[cfg(target_os = "ios")]
...
pub fn platform_webview_version() -> Result<String> {
  // NSBundle::bundleWithIdentifier("com.apple.WebKit") crashes inside CoreFoundation
  // (CFRelease) on iOS 26.5+ (both simulator and real device).
  // Use OS version as a proxy for WebKit version on iOS, since WebKit version
  // closely tracks the iOS version.
  // See: https://forums.developer.apple.com/forums/thread/809860
  #[cfg(target_os = "ios")]
  {
    let os_version = unsafe { NSProcessInfo::processInfo().operatingSystemVersionString() };
    return Ok(format!("WebKit (iOS {})", os_version));
  }
